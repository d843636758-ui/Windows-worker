...
export const definitions: Def[] = [
  ...
  {name:"taobao_native_status",description:"Check whether the official Taobao desktop client CLI is installed, running, and logged in.",schema:{}},
  {name:"taobao_native_search",description:"Search products through the logged-in official Taobao desktop client. Search prices are starting prices; inspect SKU choices before price comparisons.",schema:{keyword:z.string().min(1).max(200),type:z.enum(["all","shop","tmall","22pc_b","pc_taobao"]).default("all")}},
  {name:"taobao_native_open_product",description:"Open a known official Taobao or Tmall product URL in the desktop client.",schema:{url:z.string().url()}},
  {name:"taobao_native_read_page",description:"Read visible text from the current Taobao desktop page.",schema:{scope:z.string().max(500).optional(),max_length:z.number().int().min(500).max(30000).default(12000),offset:z.number().int().min(0).default(0)}},
  {name:"taobao_native_get_skus",description:"Get every SKU dimension and option for the current or specified product. Call this before adding anything to cart.",schema:{item_id:z.string().max(100).optional()}},
  {name:"taobao_native_get_sku_price",description:"Select exact SKU options by DOM index, wait for Taobao's asynchronous refresh, and return the real current SKU price controls. Use this for every price comparison.",schema:{options:z.array(z.string().min(1).max(200)).min(1).max(12)}},
  {name:"taobao_native_add_to_cart",description:"Add a fully specified SKU to the Taobao cart. The first call returns a confirmation token; repeat with that token after explicit user approval.",schema:{item_id:z.string().max(100).optional(),sku:z.array(z.string().min(1).max(200)).max(12).default([]),confirmation_token:z.string().optional()}},
  {name:"taobao_native_open_chat",description:"Open WangWang from cart, order, or search and send a message. The first call only previews and returns a confirmation token.",schema:{source:z.enum(["cart","order","search"]),message:z.string().min(1).max(3000),product_name:z.string().max(300).optional(),query:z.string().max(300).optional(),confirmation_token:z.string().optional()}},
  {name:"taobao_native_send_chat_message",description:"Send another WangWang message in the open conversation. The first call only previews and returns a confirmation token.",schema:{message:z.string().min(1).max(3000),shop_name:z.string().max(300).optional(),confirmation_token:z.string().optional()}},
];
